...
public sealed class RoomInteractionService : IRoomInteractionService
{
    ...
    private async ValueTask<string> SendLinkAsync(
        RoomId roomId,
        RoomActorState actor,
        string[] arguments,
        CancellationToken cancellationToken)
    {
        ...
        {
            return "Usage: :link <url>";
        }

        string rawLink = NormalizeChatText(string.Join(' ', arguments));
        if (string.IsNullOrWhiteSpace(rawLink))
        {
            return "Link cannot be empty.";
        }

        if (!TryNormalizeLink(rawLink, out string? normalizedLink))
        {
            return "Only valid http or https links are allowed.";
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (messageKind == RoomChatMessageKind.Whisper)
        {
            await _roomRuntimeRepository.AppendPrivateChatMessageAsync(
                roomId,
                actor.ActorId,
                actor.DisplayName,
                targetActor.ActorId,
