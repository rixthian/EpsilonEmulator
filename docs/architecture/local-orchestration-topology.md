...
This document translates the useful service topology patterns observed in `nitro-docker` into Epsilon's own architecture language.

It does not make `nitro-docker` a runtime dependency. It defines how Epsilon should run locally and later in production-like environments while preserving strict CMS, launcher, loader, runtime, asset, and data boundaries.
...
| --- | ---: | --- | --- | --- |
| `cms/system` | `4100` | Public portal, registration, login, community/home content, launcher access code display | CMS backend config, launcher URL | `cms` |
| `Epsilon.Launcher` | `5001` | Access-code redemption, launch profiles, client package routing, launcher telemetry | Gateway, assets | `nitro` config surface plus launcher handoff |
| `Epsilon.Gateway` | `5000` or `5100` | HTTP runtime APIs, bootstrap, future realtime gateway, runtime command authority | PostgreSQL/Redis when enabled | `arcturus` |
| Future `Runtime WSS` | `wss://localhost:*` | Authoritative room session transport | Gateway/runtime domain | `arcturus` WebSocket |
...
| `cms` | Node.js runtime for `cms/system` | Public portal only. |
...
- Do not make AtomCMS or any third-party CMS the Epsilon architecture.
...
3. Add asset-service manifest endpoints before final Unity/Nitro-style loader work.
