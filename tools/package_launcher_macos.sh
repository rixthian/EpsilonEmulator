...
LAUNCHER_DIR="$ROOT_DIR/apps/epsilon-launcher-native"
PROJECT_FILE="$LAUNCHER_DIR/EpsilonLauncher.vbproj"
PUBLISH_DIR="$LAUNCHER_DIR/bin/Release/net8.0/osx-arm64/publish"
DIST_DIR="$LAUNCHER_DIR/dist/macos"
APP_DIR="$DIST_DIR/EpsilonLauncher.app"
ICONSET_DIR="$DIST_DIR/EpsilonLauncher.iconset"
STAGING_DIR="$DIST_DIR/staging"
DMG_PATH="$LAUNCHER_DIR/dist/EpsilonLauncher-macOS-arm64.dmg"
ICON_SOURCE="$LAUNCHER_DIR/Assets/app-icon.png"
VERSION="$(ROOT_DIR="$ROOT_DIR" python3 - <<'PY'
import os
from pathlib import Path
import re
text = Path(os.environ["ROOT_DIR"], "Directory.Build.props").read_text()
